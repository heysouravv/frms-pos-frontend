...
function CustomTable({ list, total }) {
	...
	const columns = [
		...
		},
		{
			...
			key: "name",
			render: (name, { id }) => (
				<Link to={`/product-subcategory/${id}`}>{name}</Link>
			),
		},
		{
			title: "Created At",
		...
		<div className='card column-design'>
			<div className='card-body'>
				<div className='card-title d-flex justify-content-between'>
					<h5 className="font-semibold">Subcategory List</h5>
					{list && (
						<div>
							<CSVLink
								data={list}
								className=' bg-[#FE4F00]  flex items-center gap-x-2 hover:bg-none capitalize text-white  px-[14px] rounded-sm py-[5px] text-sm mb-1'
								filename='customer'>
              ...
              Column Visibility</Button>
				...
				<Table
					...
					pagination={{
						...
						onChange: (page, limit) => {
							dispatch(loadAllProductSubCategory({ page, limit }));
						},
					}}
...
const GetAllProductSubCategory = (props) => {
	...
	const list = useSelector((state) => state.productSubCategories.list);

	useEffect(() => {
		dispatch(loadAllProductSubCategory({ page: 1, limit: 10 }));
	}, []);
