...
function CustomTable({ list, total }) {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "name",
      render: (name, { id }) => (
        <Link to={`/product-brand/${id}`}>{name}</Link>
      ),
    },
    {
      title: "Created At",
    ...
    <div className="card column-design">
      <div className="card-body">
        <div className="card-title d-flex justify-content-between">
          <h5 className="font-semibold">Brand List</h5>
          {list && (
            <div>
              <CSVLink
                data={list}
        ...
        <Table
          ...
          pagination={{
            ...
            onChange: (page, limit) => {
              dispatch(loadAllProductBrand({ page, limit }));
            },
          }}
...
const GetAllProductBrand = (props) => {
  const dispatch = useDispatch();
  const list = useSelector((state) => state.productBrands?.list);

  useEffect(() => {
    dispatch(loadAllProductBrand({ page: 1, limit: 10 }));
  }, []);
